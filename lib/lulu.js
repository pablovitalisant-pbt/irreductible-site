...
import { BOOK_SKU, BOOK_TITLE, BOOK_AUTHOR, BOOK_PAGE_COUNT, SHIPPING_LEVEL_MAP, FULFILLMENT_FEE_USD } from './constants.js';
...
const PLACEHOLDER_ADDRESS = {
  US: { city: 'New York', street1: '123 Main St', postcode: '10001', state_code: 'CA', phone_number: '+12125551234' },
  CA: { city: 'Toronto', street1: '123 Queen St', postcode: 'M5V 2A1', phone_number: '+14165551234' },
  MX: { city: 'Ciudad de Mexico', street1: 'Av. Reforma 123', postcode: '06600', phone_number: '+525555123456' },
  ES: { city: 'Madrid', street1: 'Calle Mayor 1', postcode: '28013', phone_number: '+34915551234' },
...
export async function calculateCosts({ country, shipping_option, quantity }) {
  ...
  try {
    ...
    const unitPrice = parseFloat(estimate.line_item_costs[0].cost_excl_discounts);
    const printingTotal = parseFloat(estimate.line_item_costs[0].total_cost_incl_tax);
    const shippingUsd = parseFloat(estimate.shipping_cost.total_cost_incl_tax);
    const fulfillment = parseFloat(estimate.fulfillment_cost.total_cost_incl_tax);
    const total = parseFloat(estimate.total_cost_incl_tax);
    ...
    return {
      ...
      breakdown: {
        ...
        quantity,
        printing_total_usd: Math.round(printingTotal * 100) / 100,
        fulfillment_fee_usd: Math.round(fulfillment * 100) / 100,
        shipping_usd: Math.round(shippingUsd * 100) / 100,
        total_usd: Math.round(total * 100) / 100,
      }
    };
  } catch (e) {
    apiError = e.message;
    console.error('[lulu/calculate] API cost estimate falló, usando valores hardcodeados:', apiError);
  }
  ...
  const unitPrice = process.env.BOOK_UNIT_COST_USD
    ...
    : Math.round((6.00 + Math.max(0, BOOK_PAGE_COUNT * 0.02)) * 100) / 100;

...
export async function createPrintJob({ quantity, buyer_name, buyer_email, shipping_address, shipping_option }) {
  ...
  // Lulu limita street1 a 30 caracteres. Si line1 excede, truncamos y
  // movemos el resto a street2 para no perder información.
  let street1 = (shipping_address.line1 || '').trim();
  let street2 = (shipping_address.line2 || '').trim();
  if (street1.length > 30) {
    const overflow = street1.substring(30).trim();
    street1 = street1.substring(0, 30).trim();
    street2 = street2 ? overflow + ', ' + street2 : overflow;
  }

...
export async function getAccessToken() {
  ...
  const clientKey = (process.env.LULU_CLIENT_KEY || '').trim();
  const clientSecret = (process.env.LULU_CLIENT_SECRET || '').trim();

  if (!clientKey || !clientSecret) {
    throw new Error('LULU_CLIENT_KEY y LULU_CLIENT_SECRET no configurados');
  }

  console.log('[lulu] Auth URL:', LULU_AUTH);
  console.log('[lulu] LULU_CLIENT_KEY set:', !!clientKey, '| prefix:', clientKey.substring(0, 6) + '...');
  console.log('[lulu] LULU_CLIENT_SECRET set:', !!clientSecret);
...
export async function createPrintJob({ quantity, buyer_name, buyer_email, shipping_address, shipping_option }) {
  ...
  const printJob = {
    ...
    line_items: [{
      ...
      quantity,
      title: BOOK_TITLE,
      interior: process.env.BOOK_INTERIOR_URL,
      cover: process.env.BOOK_COVER_URL,
      pod_package_id: podPackageId,
    }],
    shipping_address: {
      name: buyer_name,
      street1: shipping_address.line1,
      ...(shipping_address.line2 ? { street2: shipping_address.line2 } : {}),

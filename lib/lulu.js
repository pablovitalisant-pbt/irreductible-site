...
async function getCostEstimate({ country, shipping_level, quantity }) {
  ...
  const res = await fetch(`${LULU_API}/print-job-cost-estimates/`, {
    method: 'POST',
...
export async function calculateCosts({ country, shipping_option, quantity }) {
  ...
  try {
    ...
    const unitPrice = parseFloat(estimate.line_item_costs[0].unit_cost);
    const printingTotal = parseFloat(estimate.line_item_costs[0].total_cost);
    const shippingUsd = parseFloat(estimate.shipping_cost.total_cost);
    const fulfillment = parseFloat(estimate.fulfillment_cost.total_cost);
    const total = parseFloat(estimate.total_cost);

...
export async function createOrder({ total_usd, breakdown, buyer_email, buyer_name, return_url, cancel_url, shipping_address }) {
  ...
  if (shipping_address) {
    purchaseUnit.shipping = {
      ...
      address: {
        address_line_1: (shipping_address.line1 || '').substring(0, 100),
        address_line_2: (shipping_address.line2 || '').substring(0, 100),
        admin_area_1: (shipping_address.state || '').substring(0, 60),
        admin_area_2: (shipping_address.city || '').substring(0, 60),
        postal_code: (shipping_address.postal_code || '').substring(0, 10),
